                                ...
                                <div className="w-full md:w-2/3 lg:p-8 p-6 md:p-12 pb-8 bg-white relative overflow-y-auto">
                                    ...
                                    <div className="space-y-8 mt-4">
                                        <section>
                                            ...
                                            <motion.p layoutId={`desc-${selectedId}`} className="text-sm text-gray-700 leading-relaxed">
...
export default function ServicesList() {
    ...
    // Find the currently selected service object based on the ID
    const selectedService = services.find(s => s.id === selectedId);

    useEffect(() => {
        const serviceParam = searchParams.get('service');
        if (serviceParam && !hasScrolledRef.current) {
            // Find service matching the title from URL (handling spaces/encoding)
            ...
            if (found) {
                setSelectedId(found.id);
                hasScrolledRef.current = true;
                // Optional: Ensure the view is at the top when auto-opening from another page
                        ...
                        >
                            ...
                            <div className="w-full md:w-2/3 lg:p-8 p-6 md:p-12 pb-8 bg-white relative overflow-y-auto">
                                ...
                                <div className="space-y-8 mt-4">
                                    ...
                                    {/* Key Capabilities Staggered List */}
                                    <section>
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-y-3 gap-x-6">
                                            {selectedService.details.map((detail, i) => (
                                                <motion.div
                                                    ...
                                                    initial={{ opacity: 0, x: 20 }}
                                                    animate={{ opacity: 1, x: 0 }}
                                                    transition={{ delay: 0.2 + (i * 0.1) }} // Staggered entry
                                                    className="flex items-center gap-3 text-gray-600 text-sm"
                                                >
                                                    <CheckCircle2 size={18} className="text-brand-green shrink-0" />
                                                    <span>{detail}</span>
                                    ...
                                    {/* Footer Action */}
                                    <div className="pt-6 border-t border-gray-100">
                                        <button className="w-full md:w-auto bg-slate-900 text-white px-8 py-3 rounded-xl font-bold hover:bg-brand-green transition-all duration-300 transform active:scale-95">
                                            Inquire About {selectedService.title}
                                        </button>
                                    </div>
                                </div>
